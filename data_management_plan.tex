...
Reverse traceroute measurements are publicly available via MLab and Google Cloud storage, and MLab makes them available via BigQuery upon request. User-side IP addresses are anonymoized before the data is published. The code is open-sourced on GitHub.

Beegol data will consist of active measurements similar to RIPE Atlas. We will anonymize client-side IP addresses. We will store the data in a format compatible with RIPE Atlas and host it in cloud-storage. 
